package com.example.trackinggoals.screens.goals.picture
...
class PictureChoiceFragment : Fragment() {
        ...
        adapter = PictureAdapter(navigator() , object : PictureActionListener {
            override fun selectPicture(pictureValues: PictureValues) {
                parentFragmentManager.setFragmentResult(requestKey, bundleOf(
                    KEY_PICTURE_RESPONSE to pictureValues.path)
                )
                navigator().goBack()
            }
        })
    ...
    companion object {
        private val IMAGES = listOf<String>(
            "https://images.unsplash.com/photo-1619362280026-accdce53f512?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1191&q=80",
        ...
        fun newInstance(requestKey: String,goalsId: Int): PictureChoiceFragment {
            ...
            fragment.arguments = bundleOf(
                ARG_REQUEST_PICTURE_KEY to requestKey,
                ARG_REQUEST_GOALS_ID to goalsId)
            return fragment
        }
